...
- Somnia Testnet contracts: registry, order book, treasury policy, spend cards, invoices, privacy vault, operator controls, risk oracle

## Private Beta Flow

1. Operator joins the private beta channel.
...
6. Operator uses invoices, SOMUSD cards, privacy intents, and audit exports to manage day-to-day activity.
...
- Local MCP works with stdio hosts such as Claude Desktop and Cursor. Browser/ChatGPT-style usage should use OpenAPI, `llms.txt`, and the HTTP developer tools until a hosted remote MCP endpoint is shipped.
